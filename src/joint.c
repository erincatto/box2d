...
static b2Joint* b2CreateJoint( b2World* world, const b2JointDef* def, b2JointType type )
{
	...
	if ( bodyA->setIndex == b2_disabledSet || bodyB->setIndex == b2_disabledSet )
	{
		...
		joint->localIndex = set->jointIds.count;
		b2Array_Push( set->jointIds, jointId );
	}
	...
	joint->constraintHertz = def->constraintHertz;
	joint->constraintDampingRatio = def->constraintDampingRatio;
	joint->constraintSoftness = (b2Softness){
		.biasRate = 0.0f,
		.massScale = 1.0f,
		.impulseScale = 0.0f,
	};
...
b2JointId b2CreateDistanceJoint( b2WorldId worldId, const b2DistanceJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_distanceJoint );
	...
	b2JointId jointId = { joint->jointId + 1, world->worldId, joint->generation };
	return jointId;
}
...
b2JointId b2CreateMotorJoint( b2WorldId worldId, const b2MotorJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_motorJoint );
...
b2JointId b2CreateFilterJoint( b2WorldId worldId, const b2FilterJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_filterJoint );
...
b2JointId b2CreatePrismaticJoint( b2WorldId worldId, const b2PrismaticJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_prismaticJoint );
...
b2JointId b2CreateRevoluteJoint( b2WorldId worldId, const b2RevoluteJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_revoluteJoint );
...
b2JointId b2CreateWeldJoint( b2WorldId worldId, const b2WeldJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_weldJoint );
...
b2JointId b2CreateWheelJoint( b2WorldId worldId, const b2WheelJointDef* def )
{
	...
	b2Joint* joint = b2CreateJoint( world, &def->base, b2_wheelJoint );
...
void b2DestroyJointInternal( b2World* world, b2Joint* joint, bool wakeBodies )
{
	...
	b2SolverSet* set = b2SolverSetArray_Get( &world->solverSets, setIndex );
	int lastIndex = set->jointIds.count - 1;
	if ( localIndex < lastIndex )
	{
		int movedId = set->jointIds.data[lastIndex];
		set->jointIds.data[localIndex] = movedId;
		b2Joint* movedJoint = b2JointArray_Get( &world->joints, movedId );
		B2_ASSERT( movedJoint->localIndex == lastIndex );
		movedJoint->localIndex = localIndex;
	}
	set->jointIds.count -= 1;
...
static b2Vec2 b2GetJointConstraintForce( b2World* world, b2Joint* joint )
{
	...
	{
		case b2_distanceJoint:
			return b2GetDistanceJointForce( world, joint );

		case b2_motorJoint:
			return b2GetMotorJointForce( world, joint );

		case b2_filterJoint:
			return b2Vec2_zero;

		case b2_prismaticJoint:
			return b2GetPrismaticJointForce( world, joint );

		case b2_revoluteJoint:
			return b2GetRevoluteJointForce( world, joint );

		case b2_weldJoint:
			return b2GetWeldJointForce( world, joint );

		case b2_wheelJoint:
			return b2GetWheelJointForce( world, joint );
...
static float b2GetJointConstraintTorque( b2World* world, b2Joint* joint )
{
	switch ( joint->type )
	{
		...
		case b2_motorJoint:
			return b2GetMotorJointTorque( world, joint );

		case b2_filterJoint:
			return 0.0f;

		case b2_prismaticJoint:
			return b2GetPrismaticJointTorque( world, joint );

		case b2_revoluteJoint:
			return b2GetRevoluteJointTorque( world, joint );

		case b2_weldJoint:
			return b2GetWeldJointTorque( world, joint );

		case b2_wheelJoint:
			return b2GetWheelJointTorque( world, joint );
...
float b2Joint_GetLinearSeparation( b2JointId jointId )
{
	...
	b2Vec2 pA = b2TransformPoint( xfA, joint->localFrameA.p );
	b2Vec2 pB = b2TransformPoint( xfB, joint->localFrameB.p );
	...
	switch ( joint->type )
	{
		case b2_distanceJoint:
		{
			b2DistanceJoint* distanceJoint = &joint->distanceJoint;
		...
		case b2_prismaticJoint:
		{
			b2PrismaticJoint* prismaticJoint = &joint->prismaticJoint;
		...
		case b2_weldJoint:
		{
			b2WeldJoint* weldJoint = &joint->weldJoint;
		...
		case b2_wheelJoint:
		{
			b2WheelJoint* wheelJoint = &joint->wheelJoint;
			b2Vec2 axisA = b2RotateVector( xfA.q, (b2Vec2){ 1.0f, 0.0f } );
			b2Vec2 perpA = b2LeftPerp( axisA );
			float perpendicularSeparation = b2AbsFloat( b2Dot( perpA, dp ) );
			float limitSeparation = 0.0f;
...
float b2Joint_GetAngularSeparation( b2JointId jointId )
{
	...
	switch ( joint->type )
	{
		...
		case b2_revoluteJoint:
		{
			b2RevoluteJoint* revoluteJoint = &joint->revoluteJoint;
...
void b2Joint_GetConstraintTuning( b2JointId jointId, float* hertz, float* dampingRatio )
{
	b2World* world = b2GetWorld( jointId.world0 );
	b2Joint* joint = b2GetJointFullId( world, jointId );
	*hertz = joint->constraintHertz;
	*dampingRatio = joint->constraintDampingRatio;
}
...
void b2Joint_SetForceThreshold( b2JointId jointId, float threshold )
{
	...
	b2World* world = b2GetWorld( jointId.world0 );
	b2Joint* joint = b2GetJointFullId( world, jointId );
	joint->forceThreshold = threshold;
}
...
void b2Joint_SetTorqueThreshold( b2JointId jointId, float threshold )
{
	...
	b2World* world = b2GetWorld( jointId.world0 );
	b2Joint* joint = b2GetJointFullId( world, jointId );
	joint->torqueThreshold = threshold;
}
...
void b2DrawJoint( b2DebugDraw* draw, b2World* world, b2Joint* joint )
{
	...
	b2Vec2 pA = b2TransformPoint( transformA, joint->localFrameA.p );
	b2Vec2 pB = b2TransformPoint( transformB, joint->localFrameB.p );
	...
	switch ( joint->type )
	{
		...
		case b2_prismaticJoint:
			b2DrawPrismaticJoint( draw, joint, transformA, transformB, scale );
			break;

		case b2_revoluteJoint:
			b2DrawRevoluteJoint( draw, joint, transformA, transformB, scale );
			break;

		case b2_weldJoint:
			b2DrawWeldJoint( draw, joint, transformA, transformB, scale );
			break;

		case b2_wheelJoint:
			b2DrawWheelJoint( draw, joint, transformA, transformB, scale );
			break;
