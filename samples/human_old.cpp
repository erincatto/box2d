...
void HumanOld::Spawn( b2WorldId worldId, b2Vec2 position, float scale, float frictionTorque, float hertz, float dampingRatio,
...
void HumanOld::ApplyRandomAngularImpulse( float magnitude )
{
...
void HumanOld::SetJointFrictionTorque( float torque )
{
	if ( m_isSpawned == false )
	{
		return;
...
void HumanOld::SetJointSpringHertz( float hertz )
{
	if ( m_isSpawned == false )
	{
		return;
...
void HumanOld::SetJointDampingRatio( float dampingRatio )
{
	if ( m_isSpawned == false )
	{
		return;
