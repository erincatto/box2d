...
void CreateJointGrid( b2WorldId worldId )
{
	...
	for ( int k = 0; k < N; ++k )
	{
		for ( int i = 0; i < N; ++i )
		{
			...
			bodyDef.position = b2MakePosition( (b2Vec2){ fk, -fi } );
...
void CreateLargePyramid( b2WorldId worldId )
{
	...
	for ( int i = 0; i < baseCount; ++i )
	{
		...
		for ( int j = i; j < baseCount; ++j )
		{
			...
			bodyDef.position = b2MakePosition( (b2Vec2){ x, y } );
...
void CreateSpinner( b2WorldId worldId )
{
...

	{
		b2BodyDef bodyDef = b2DefaultBodyDef();
		bodyDef.type = b2_dynamicBody;
		bodyDef.position = b2MakePosition( (b2Vec2){ 0.0, 12.0f } );
		bodyDef.enableSleep = false;
...
void CreateSmash( b2WorldId worldId )
{
...

	{
		...
		b2BodyDef bodyDef = b2DefaultBodyDef();
		bodyDef.type = b2_dynamicBody;
		bodyDef.position = b2MakePosition( (b2Vec2){ -20.0f, 0.0f } );
...
void CreateTumbler( b2WorldId worldId )
{
...

	{
		b2BodyDef bodyDef = b2DefaultBodyDef();
		bodyDef.type = b2_dynamicBody;
		bodyDef.position = b2MakePosition( (b2Vec2){ 0.0f, 10.0f } );
		b2BodyId bodyId = b2CreateBody( worldId, &bodyDef );
...
void CreateJunkyard( b2WorldId worldId )
{
	...
	for ( int i = 0; i < columnCount; ++i )
	{
		...
		for ( int j = 0; j < rowCount; ++j )
		{
			...
			bodyDef.position = b2MakePosition( (b2Vec2){ x + side, y } );
			side = -side;
	...
	bodyDef.type = b2_kinematicBody;
	bodyDef.position = b2MakePosition( b2Vec2_zero );
